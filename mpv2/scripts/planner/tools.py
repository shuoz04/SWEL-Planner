#!/usr/bin/env python3
"""Small shared helpers for unit conversion and MoveIt status handling."""
...
AngleLike = Union[float, list, tuple, dict, np.ndarray]


def _convert_angle(value: AngleLike, factor: float) -> AngleLike:
...
def rad2deg(rad: AngleLike) -> AngleLike:
    return _convert_angle(rad, 180 / pi)


def deg2rad(deg: AngleLike) -> AngleLike:
    return _convert_angle(deg, pi / 180)


def wrap2pi(rad: float | list | tuple) -> float | list:
    if isinstance(rad, (list, tuple)):
        return [wrap2pi(value) for value in rad]
...
class Constant:
    moveit_error_code_map = MOVEIT_ERROR_CODE_MAP


def moveit_error_code_to_string(error_code: moveit_msgs.msg.MoveItErrorCodes) -> str:
    return Constant.moveit_error_code_map.get(error_code.val, "unknown error")
